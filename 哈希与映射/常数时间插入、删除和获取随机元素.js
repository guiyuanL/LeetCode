...
RandomizedSet.prototype.insert = function (val) {
  if (!this.dict.has(val)) {
    this.dict.set(val, this.arr.length)
    this.arr.push(val)
    return true
  } else {
    return false
...
RandomizedSet.prototype.remove = function (val) {
  ...
  let index = this.dict.get(val)
  let lastEle = this.arr[n]
  this.arr[n] = this.arr[index]
  this.arr[index] = lastEle
  this.dict.set(this.arr[index], index)
  this.arr.pop()
